...
\begin{center}
\textcolor{red}{%
In a model coupling fungal growth with standard decomposition reaction kinetics, we study the effect of varying parameters of fungal growth on the overall decomposition of a wood-based substrate by 35 species. 
Growth is simulated using a 1-dimensional growth model based on fundamental microscopic growth mechanisms, linearly coupled with a Michaelis-Menten reaction equation. Interactions between different fungi was limited to competition for a common nutrient pool. Various fungal species were differentiated by their enzymic makeup of four different representative enzymes and their ecological niches. The parameters of moisture potential and temperature, combined with data on hyphal extension rates, were used to represent the influence of the environment across the model. We studied two areas: the relative advantages of individual fungi and the effect of biodiversity on the community decomposition efficiency. Our results support the existence of a spectrum where species either tend towards being more competitive, demonstrating a higher hyphal extension and decomposition rate, or more stress tolerant, demonstrating less variability facing long term environmental fluctuations. The introduction of yearly moisture potential fluctuations corresponded with a slight increase in overall decomposition system efficiency. The positive impact of biodiversity distribution correlated with competitive ranking coefficients up to a critical point, where further following competitive ranking coefficient distribution appeared to have no correspondence with the  total decomposition efficiency. The gap in understanding between short and long term fluctuations of environmental parameters and this critical point of biodiversity distribution prompt further investigation.}
\end{center}
% to here
%%%%%%%%%%% End Summary %%%%%%%%%%%
...
\captionof{figure}[Average total decomposition against a]{Average total decomposition as seen in Figure \ref{Carbon Decrease Fungi Distribution} plotted against ranking distribution coefficient for a certain species distribution.}
\end{center}
\begin{multicols}{2}
